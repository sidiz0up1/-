...
export default function App() {
  ...
  const handleAddStore = () => {
    ...
    const newStore = storageService.addStore(newStoreName.trim());
  ...
  const handleUpdateStore = (id: string, name: string) => {
    if (!name.trim()) return;
    storageService.updateStore(id, name.trim());
  ...
  const handleDeleteStore = (id: string) => {
    ...
    if (storeToDelete?.name === selectedStore) {
      setSelectedStore(stores.find(s => s.id !== id)?.name || '');
    }
  };
    ...
    try {
      ...
      for (let i = 0; i < pageIds.length; i++) {
        ...
        const dataUrl = await htmlToImage.toPng(element, {
          quality: 0.95,
          pixelRatio: 1.5, // Slightly lower for better compatibility
