...
export const HistoryView: React.FC = () => {
  ...
  const handleTogglePurchase = async (id: string, currentStatus: boolean) => {
    try {
      ...
      if (success) {
        setRecords(prev => {
          const newRecords = prev.map(r => r.id === id ? { ...r, purchased: !currentStatus } : r);
          console.log(`Successfully updated record ${id} purchase status to ${!currentStatus}`);
          return newRecords;
        });
      } else {
        console.error(`Failed to update record ${id} purchase status. storageService returned false.`);
        alert('구매 상태 업데이트에 실패했습니다. DB 설정을 확인해주세요.');
      }
    } catch (err) {
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2 pr-4 border-r border-slate-100">
                    <span className={`text-[11px] font-bold ${record.purchased ? 'text-green-600' : 'text-slate-400'}`}>
